...
const paginatedDogs = computed(() => {
  ...
  return dogs.value.slice(start, start + pageSize)
})
...
<template>
  <div
    v-if="loading"
    class="w-full h-screen flex items-center justify-center -translate-y-24"
  >
    <div role="status">
      <svg
        aria-hidden="true"
  ...
  <div v-else class="grid gap-4 grid-cols-1 md:grid-cols-3 mb-6">
    <BreedCard
  ...
  <Pagination
    v-if="dogs.length > 10"
    v-bind:total-pages="Math.ceil(dogs.length / pageSize)"
